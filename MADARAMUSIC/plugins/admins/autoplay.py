...
async def autoplay_mode(client, message: Message, _, chat_id):
    # Agar user direct /autoplay enable ya /autoplay disable likhta hai
    if len(message.command) > 1:
        arg = message.command[1].lower()
        if arg in ["enable", "on", "true"]:
            await set_autoplay(chat_id, True)
            return await message.reply_text("✅ Auto Play is now **Enabled** for this chat.")
        elif arg in ["disable", "off", "false"]:
            await set_autoplay(chat_id, False)
            return await message.reply_text("❌ Auto Play is now **Disabled** for this chat.")
    
    # Agar user sirf /autoplay likhta hai toh panel khulega
